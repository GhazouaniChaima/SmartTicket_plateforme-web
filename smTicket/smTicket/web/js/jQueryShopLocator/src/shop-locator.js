...
;(function ( $, window, document, undefined ) {
    ...
    var pluginName = "ShopLocator",
        defaults = {
            ...
            waypoints:{
                enable: true,
                json: "./assets/js/jQueryShopLocator/src/json/waypoints.json"
            },
            infoBubble:{
                visible: false,
                padding: 0,
    ...
    var g_openInfoWindow, triggerCurrentSlide;
    ...
    $.extend(Plugin.prototype, {
        ...
        setUpMap: function (element, settings) {
            ...
            if(settings.waypoints.enable == true){
                this.displayWaypoints(this, element, mapCanvas, bounds, settings)
            }else{
        ...
        createMarkers: function(map, searchBox, markerTable, settings){
            ...
            if(settings.infoBubble.visible == true) {
                ...
                google.maps.event.addListener(marker, "click", function () {
                    if(g_openInfoWindow){
                        g_openInfoWindow.close()
                    }
                    infoBubble.open(map, marker);
                    g_openInfoWindow = infoBubble;
                });
                if(settings.infoBubble.getDirectionsButton == true){
        ...
        markerWaypoints: function(map, jsonData, position, counter, settings){
            ...
            infoBubble = new InfoBubble({
                visible: settings.infoBubble.visible,
                content: "<div class = 'ct-googleMap--InfoWindowBody' style='text-align: center;'>"+
                "<h6 class='ct-u-marginBottom10 ct-u-colorMotive text-uppercase'>"+jsonData.windowTitle+"</h6>"+
                "<a href='#'>"+jsonData.windowDescription+"</span>",
                backgroundClassName: 'ct-googleMap--customInfoWindow',
                padding: settings.infoBubble.padding,
                borderRadius: settings.infoBubble.borderRadius,
                borderWidth: settings.infoBubble.borderWidth,
            ...
            google.maps.event.addListener(marker, "click", function () {
                $(".ct-js-slick-googleMaps").slick("slickGoTo", counter);
                if(g_openInfoWindow){
                    g_openInfoWindow.close()
                }
                triggerCurrentSlide = counter;
                infoBubble.open(map, marker);
                g_openInfoWindow = infoBubble;
            });
            return marker
        },
        displayWaypoints: function(constructor, element, map, bounds, settings){
            $.ajax({
                url: settings.waypoints.json,
                dataType: 'json',
                success: function(data) {
                    ...
                    directionsService.route(request, function(response, status) {
                        if (status == google.maps.DirectionsStatus.OK) {
                            ...
                            for (var i = 0; i < route.length; i++) {
                                marker = constructor.markerWaypoints(map, data[i], route[i].start_location, i, settings);
                                markersArray.push(marker);
                                if(i+1== route.length){
                                    marker = constructor.markerWaypoints(map, data[i], route[i].end_location, route.length, settings);
                                    markersArray.push(marker);
                                }
                            }
                        }
                    });
                    $('.ct-js-slick-googleMaps').on('afterChange', function(event, slick, currentSlide, nextSlide){
                        var $map = $(".ct-js-googleMap--waypoints .ct-js-googleMap");
                        var slickSlide = $(".ct-js-slick-googleMaps").slick("slickCurrentSlide");
                        var activeMarker = markersArray[slickSlide];
                        if(triggerCurrentSlide != slickSlide){
